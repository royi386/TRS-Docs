...
// Scanning apps (CamScanner, Adobe Scan, phone exports…) stamp every page with
// a watermark text run and nothing else. The PDF then "has" a text layer, but
// it holds none of the page's content, so it must never be trusted as one.
const OCR_WATERMARK_PATTERNS = [
  /\bScanned by CamScanner\b/i,
  /\bScanned with CamScanner\b/i,
  /\bCamScanner\b/i,
  /\bScan.{0,3}ed? with (a )?(Scanner|Adobe Scan)\b/i,
  /\bAdobe Scan\b/i,
  /^\s*(Generated by|Powered by) .{0,40}(Scanner|Scan)\s*$/i,
  /-{2,}\s*Page \d+\s*-{2,}/
];
// A page whose whole text layer is this short is a stamp, not content.
const OCR_STAMP_PAGE_MAX_CHARS = 60;
 ...
 * stamps come back as one long run), or a few stray characters. Such a page
 * must be read from its image, not trusted from the layer.
 */
function isStampOnlyPageText(text) {
  const value = String(text || '').trim();
  if (!value) return true;
  // Remove every stamp and separator; whatever survives is the page's content.
  let remainder = value;
  for (const pattern of OCR_WATERMARK_PATTERNS) {
    remainder = remainder.replace(new RegExp(pattern.source, pattern.flags.includes('g') ? pattern.flags : `${pattern.flags}g`), ' ');
  }
  remainder = remainder.replace(/[\s\-–—_=*~]+/g, ' ').trim();
  if (!remainder) return true;
  // A few stray characters after the stamps (a date, a page number) are still
  // no content; a real sentence is longer.
  if (remainder.length < 8) return true;
  // A short page whose leftover text reads as garbage is handed to OCR as well.
  if (value.length <= OCR_STAMP_PAGE_MAX_CHARS && !isReadableText(remainder)) return true;
  return false;
}
...
function getConfig() {
  ...
  return {
    ...
    // Hard ceiling for safety; most deployments set RAG_OCR_MAX_PAGES higher
    // than the 40-page default because full scanned books are common here.
    ocrMaxPages: Math.min(readNumber(process.env.RAG_OCR_MAX_PAGES, OCR_MAX_PAGES), 1000),
    ...
    // OCR every page, not only the ones with no text layer at all. This is the
    // safe default: a scanner's text layer often holds just a watermark, so a
    // page can look "readable" while all of its content lives in the image.
    // Set RAG_OCR_ALL=0 to fall back to the old behaviour (OCR only empty pages)
    // when the library is big and indexing time matters more than coverage.
    ocrAllPages: process.env.RAG_OCR_ALL !== '0',
    // Write the text layer into the stored page text alongside the OCR result.
    // The layer holds searchable wording (fonts, spacing, document numbers) that
    // Tesseract sometimes mangles, while OCR recovers what the layer never had.
    ocrMergeTextLayer: process.env.RAG_OCR_MERGE_LAYER !== '0',
 ...
 *
 * A page whose layer holds only a scanner stamp ("Scanned by CamScanner") or a
 * few stray characters is dropped here, so the caller sees the page as
 * text-less and hands it to OCR instead of trusting the watermark.
 */
    ...
    for (let pageNumber = 1; pageNumber <= pdfDocument.numPages; pageNumber += 1) {
      try {
        ...
        try {
          ...
          if (cleaned && !isStampOnlyPageText(cleaned)) {
...
/**
 * OCR pass for PDFs that already carry some native text. By default every page
 * is rasterised and read: a text layer produced by a scanning app usually holds
 * just a watermark, and even a genuine layer can miss what the page image shows
 * (stamps, margin notes, table columns). With RAG_OCR_ALL=0 only the pages whose
 * layer was dropped by the stamp filter are read. Returns null when there is
 * nothing for OCR to do.
 */
async function ocrMissingTextPages(filePath, extracted, options = {}) {
  const config = getConfig();
  if (!config.ocrEnabled) return null;
  const missing = [];
  if (config.ocrAllPages) {
    for (let pageNumber = 1; pageNumber <= extracted.pageCount; pageNumber += 1) missing.push(pageNumber);
  } else {
...
/**
 * Merges the native text layer with the OCR result for the same page. The
 * layer's wording (exact document numbers, font-exact spacing) and the OCR
 * reading (what the page image actually shows) both carry content the other
 * misses, so joined text preserves both. Keeping both readings also strengthens
 * keyword hits for the page when the two agree.
 */
function mergeLayerAndOcr(nativePages, ocrPages) {
  const merged = new Map(nativePages.map(page => [page.page, page.text]));
  for (const page of ocrPages) {
    const layer = merged.get(page.page);
    merged.set(page.page, layer ? `${layer}\n${page.text}` : page.text);
  }
...
function createRag({ db, uploadDirectory, logger = console }) {
  ...
  async function indexFile(source, { force = false, knownStat = null, reason = 'scan' } = {}) {
    ...
    // The stored revision changes when extraction itself changes (e.g. the
    // stamp filter or OCR-every-page behaviour), so documents indexed by the
    // older code are re-read even though the PDF bytes never moved.
    const revision = `${modelId}|${EXTRACTION_REVISION}`;
    if (!force
      && indexed
      && indexed.status === 'indexed'
      && indexed.size === fileStat.size
      && indexed.modified_ms === Math.round(fileStat.mtimeMs)
      && indexed.embedding_model === revision) {
    ...
    // Pages whose text layer is missing *or* just a scanner stamp are read
    // with OCR. By default every page is rasterised and read, because a layer
    // produced by a scanning app often holds nothing but a watermark while the
    // content lives in the page image. When a page has both, the text layer and
    // the OCR result are merged so neither source is lost.
    let ocrError = '';
    let ocrPages = 0;
    if (config.ocrEnabled) {
      try {
        ...
        if (ocr && ocr.pages.length) {
          if (totalChars && config.ocrMergeTextLayer) {
            pages = mergeLayerAndOcr(pages, ocr.pages);
          } else if (totalChars) {
            pages = [...pages, ...ocr.pages].sort((left, right) => left.page - right.page);
          } else {
            pages = ocr.pages;
          }
          totalChars = pages.reduce((sum, page) => sum + page.text.length, 0);
          ocrPages = ocr.pages.length;
          logger.log(`[rag] ${source}: OCR read ${ocrPages} page(s) of text`);
        }
      } catch (error) {
